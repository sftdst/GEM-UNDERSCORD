...
export default function PlanEdit() {
    const { plan, fonctionnalites } = usePage().props as unknown as {
        plan: Plan;
        fonctionnalites: Fonctionnalite[];
    };
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        stockage_max_go:   String(plan.stockage_max_go),
        users_max:         plan.users_max ? String(plan.users_max) : '',
        documents_max:     plan.documents_max ? String(plan.documents_max) : '',
        actif:             plan.actif,
        fonctionnalite_ids: (plan.fonctionnalites ?? []).map((f) => f.id),
    });
    ...
    return (
        <SuperAdminLayout>
            <Head title={`Modifier — ${plan.nom}`} />

            <div className="max-w-2xl mx-auto space-y-6">
                {/* Header */}
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Modifier le Plan</h1>
                        <p className="text-gray-600 mt-1">{plan.nom}</p>
                    </div>
